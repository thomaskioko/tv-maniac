...
@Composable
internal fun DiscoverSectionsPage(
    state: SettingsState,
    ...
    LazyColumn(modifier = modifier.testTag(SettingsTestTags.LIST_TEST_TAG)) {
        ...
        item {
            SettingsGroup {
                state.discoverSectionToggles.forEach { toggle ->
                    SwitchRow(
                        modifier = Modifier.testTag(SettingsTestTags.discoverSectionToggleTag(toggle.section.name)),
                        title = toggle.label,
                        checked = toggle.visible,
                        onCheckedChange = { onAction(DiscoverSectionToggled(toggle.section, visible = it)) },
                    )
                }
            }
        }
...
@Composable
private fun DiscoverSectionsPagePreview() {
