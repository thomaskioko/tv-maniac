...
class MyShowsPresenterTest {
    ...
    private val factory = FakeMyShowsPresenterBuilder()

    private lateinit var presenter: MyShowsPresenter
    ...
    @Test
    fun `should emit MyShowsState with content on success`() = runTest {
        presenter.state.test {
            awaitItem() shouldBe MyShowsState()
    ...
    @Test
    fun `should update query when MyShowsQueryChanged is dispatched`() = runTest {
        presenter.state.test {
            awaitItem() shouldBe MyShowsState()
            ...
            presenter.dispatch(MyShowsQueryChanged("test query"))
    ...
    fun `should filter watchNextEpisodes by show name when query is active`() = runTest {
        ...
        presenter.state.test {
            ...
            presenter.dispatch(MyShowsQueryChanged("Loki"))
