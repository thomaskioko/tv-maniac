...
class UserCacheImpl(
    ...
    override fun insert(traktUser: Trakt_user) {
        database.trakt_userQueries.insertOrReplace(
            slug = traktUser.slug,
    ...
    override fun observeUserBySlug(slug: String): Flow<Trakt_user?> {
        return database.trakt_userQueries.userBySlug(slug)
            .asFlow()
            .mapToOneOrNull(dispatchers.io)
    }

    override fun observeMe(): Flow<Trakt_user> {
        return database.trakt_userQueries.getMe()
            .asFlow()
            .mapToOne(dispatchers.io)
    }

    override fun getMe(): Trakt_user? = database.trakt_userQueries.getMe().executeAsOneOrNull()
}