...
public sealed class ShowResolveOutcome {
    public data class Resolved(public val showId: Id<ShowId>) : ShowResolveOutcome()
    public data object Skipped : ShowResolveOutcome()
}

@Inject
@SingleIn(AppScope::class)
public class ShowReconciler(
    ...
    public suspend fun reconcile(
        ...
        val showId = showIdResolver.showIdForTmdbId(resolvedTmdbId)
            ?: return ShowResolveOutcome.Skipped to result.copy(tmdbMissing = result.tmdbMissing + 1)

        return ShowResolveOutcome.Resolved(showId) to result.copy(matched = result.matched + 1)
    }
