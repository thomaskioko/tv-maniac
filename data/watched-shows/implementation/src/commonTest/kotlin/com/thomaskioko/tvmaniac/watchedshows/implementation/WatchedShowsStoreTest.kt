...
class WatchedShowsStoreTest {
    ...
    private lateinit var store: WatchedShowsStore

    @BeforeTest
    fun setUp() {
        store = WatchedShowsStore(
    ...
    fun `should write fetched rows to dao given fetch returns payload`() = runTest(testDispatcher) {
        ...
        watchedShowsDao.entriesObservable().test {
    ...
    fun `should delete rows missing from new payload given writer runs`() = runTest(testDispatcher) {
        watchedShowsDao.upsert(breakingBadEntry)
        watchedShowsDao.upsert(theWireEntry)
    ...
    fun `should skip fetch given ttl valid and no activity change`() = runTest(testDispatcher) {
        ...
        watchedShowsDao.entries().shouldBeEmpty()
    }
    ...
    fun `should fetch given force refresh bypasses validator`() = runTest(testDispatcher) {
        ...
        watchedShowsDao.entries() shouldContainExactlyInAnyOrder listOf(breakingBadEntry)
    }
...
private val breakingBadEntry = WatchedShowEntry(
    traktId = 1388,
    tmdbId = 1396,
    plays = 56,
...
private val theWireEntry = WatchedShowEntry(
    traktId = 1429,
    tmdbId = 1438,
    plays = 12,
