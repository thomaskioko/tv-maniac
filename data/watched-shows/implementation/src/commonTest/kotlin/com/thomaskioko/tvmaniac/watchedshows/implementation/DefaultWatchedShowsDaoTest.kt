...
internal class DefaultWatchedShowsDaoTest : BaseDatabaseTest() {
    ...
    private lateinit var dao: WatchedShowsDao

    @BeforeTest
    fun setUp() {
        dao = DefaultWatchedShowsDao(database, dispatchers)
    }
    ...
    fun `should replace existing entry given same trakt id`() {
        dao.upsert(breakingBadEntry)
        val updated = breakingBadEntry.copy(plays = 99, lastWatchedAt = 2_000_000L)
