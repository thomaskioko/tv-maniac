...
class AndroidTraktRefreshTokenAction(
    ...
    override suspend fun invoke(currentState: AuthState): AuthState? {
        return try {
            logger.debug("AndroidTraktRefreshTokenAction: Requesting token refresh")
            ...
            if (accessToken == null || refreshToken == null || expiresIn == null) {
                logger.error("AndroidTraktRefreshTokenAction", "Invalid response - missing tokens")
                return null
            }

            val expiresAt = Clock.System.now() + expiresIn.seconds

            logger.debug("AndroidTraktRefreshTokenAction: Token refresh successful")
            SimpleAuthState(
